...
// ISR(X, __attribute__ ((weak))) doesnt work
...
void init()
{
...
#if defined(TCCR2A) && defined(WGM21) && defined(WGM20)
	...
	// timers 1 and 0 are used for phase-correct hardware pwm
...
#if defined(TCCR0) && defined(CS22)
	sbi(TCCR0, CS01);
	sbi(TCCR0, CS00);
#elif defined(TCCR0B) && defined(CS22)
	sbi(TCCR0B, CS01);
	sbi(TCCR0B, CS00);
#else
	#error Timer 0 not finished (may not be present on this CPU)
#endif

	// configure timer 0 for phase correct pwm (8-bit)
#if defined(TCCR0) && defined(WGM20)
	sbi(TCCR0, WGM01);
	sbi(TCCR0, WGM00);
#elif defined(TCCR0A) && defined(WGM20)
	sbi(TCCR0A, WGM01);
	sbi(TCCR0A, WGM00);
#else
	#error Timer 0 not finished (may not be present on this CPU)
