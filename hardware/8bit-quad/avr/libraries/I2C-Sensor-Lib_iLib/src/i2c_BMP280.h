...
public:
  ...
  void read(float& pressure, float& temperature) {
    uint8_t raw[6];
    i2c.read(I2C_ADDRESS, REG_PRESS_MSB, raw, 6);


    // uint8_t tempRaw[3];
    // i2c.read(I2C_ADDRESS, REG_TEMP_MSB, tempRaw, 3);

    // Temperature.
    // ============
    {
      uint8_t tempRaw[3] = {raw[3], raw[4], raw[5]};
      // int32_t var1, var2, adc;
      // adc     = (uint32_t( uint16_t(tempRaw[0] << 8) | tempRaw[1]) << 4) | (tempRaw[2] >> 4);
      // var1    = ((((adc >> 3) - ((int32_t)dig_T1 << 1))) * ((int32_t)dig_T2)) >> 11;
      // var2    = (((((adc >> 4) - ((int32_t)dig_T1)) * ((adc >> 4) - ((int32_t)dig_T1))) >> 12) * ((int32_t)dig_T3)) >> 14;
      // tFine   = var1 + var2;

      int32_t adc_T = tempRaw[0]; adc_T <<= 8;
      adc_T |= tempRaw[1]; adc_T <<= 8;
      adc_T |= tempRaw[2];
      adc_T >>= 4;
      int32_t var1, var2;
    ...
    // uint8_t pressureRaw[3];
    // i2c.read(I2C_ADDRESS, REG_PRESS_MSB, pressureRaw, 3);

    // Pressure.
    // =========
    {
      uint8_t pressureRaw[3] = {raw[0], raw[1], raw[2]};
      // int32_t var1, var2, adc;
      // adc     = (uint32_t( uint16_t(pressureRaw[0] << 8) | pressureRaw[1]) << 4) | (pressureRaw[2] >> 4);
      // var1 = (((int32_t)tFine) >> 1) - (int32_t)64000;
      // var2 = (((var1 >> 2) * (var1 >> 2)) >> 11 ) * ((int32_t)dig_P6);
      // var2 = var2 + ((var1 * ((int32_t)dig_P5)) << 1);
      // var2 = (var2 >> 2) + (((int32_t)dig_P4) << 16);
      ...
      // uint32_t pascal = (((uint32_t)(((int32_t)1048576) - adc) - (var2 >> 12))) * 3125;
      // if (pascal < 0x80000000)    pascal = (pascal << 1) / ((uint32_t)var1);
      // else                        pascal = (pascal / (uint32_t)var1) * 2;
      ...
      // pressure = pascal / 100.0f;

      int64_t var1, var2, p;
      int32_t adc_P = pressureRaw[0]; adc_P <<= 8;
      adc_P |= pressureRaw[1]; adc_P <<= 8;
      adc_P |= pressureRaw[2];
      adc_P >>= 4;
